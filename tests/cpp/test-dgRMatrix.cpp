...
#pragma omp parallel
{
  ...
  std::size_t size = Xv::Xv_dgRMatrix_numeric_folded(m, x, result_element, foldid, omp_get_thread_num() + 1, false);
...
#pragma omp parallel
{
  ...
  Xv::vX_dgRMatrix_numeric_folded(m, local_x, result_element, foldid, target, false, local_row_mapping);
}
return wrap(result);
}
